...
11. Derleme ve projeyi çalıştırın. Deneysel örneğinde bir C# Evrensel hub uygulaması oluşturma, Git **Araçları** menüsüne ve ardından **çağırma TestUniversalProject**ve genel çıkış bölmesinde metnini denetleyin. Paylaşılan ilk öğenin adını proje (bekliyoruz, App.xaml dosyası) değiştirilebilir ve durumunda olduklarını görmüş olmalısınız <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> olay harekete geçirildi. Bu durumda, App.XAML yeniden adlandırma de yeniden adlandırılacak App.xaml.cs neden olduğundan, dört olayları (her platform projesi için iki) görmeniz gerekir. (DTE olayları paylaşılan projelerdeki öğelere izlemez.) İki görmelisiniz <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A> olayları (platformu projelerin her biri için bir tane), ancak Hayır <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A> olayları.  
