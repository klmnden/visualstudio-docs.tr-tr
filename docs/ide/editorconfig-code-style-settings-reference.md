...
> - Visual Studio da ayarlanabilir için kod stili tercihleri [metin düzenleyici seçenekleri](code-styles-and-code-cleanup.md) iletişim. Bununla birlikte, öncelik ve ayarladığınız tercihleri .editorconfig ayarları ele **seçenekleri** belirli bir proje ile ilişkili değildir.
...
- [Dil kod stilleri](#language-code-styles)
...
## <a name="language-code-styles"></a>Dil kod stilleri

Dil kod stili için kuralları aşağıdaki biçime sahiptir:

`options_name = false|true : none|silent|suggestion|warning|error`

Her dil kodu stil kuralı belirtmeli **true** (Bu stil tercih et) veya **false** (Bu stil tercih değil) ve bir **önem derecesi**. Önem derecesi için o stilin zorlama düzeyini belirtir.
...
Aşağıdaki listede verilen dil kod stili ayarları gösterilmektedir:
...
| csharp_style_conditional_delegate_call | IDE0041 | C# 6.0+ | TRUE: öneri |
...
**DotNet\_ayrı\_alma\_yönergesi\_grupları**

- Bu kural ayarlandığında **true**, yönerge gruplarını kullanarak arasında boş bir satıra yerleştirin.
- Bu kural ayarlandığında **false**, yönerge gruplarını kullanarak arasında boş bir satır yerleştirmeyin.
