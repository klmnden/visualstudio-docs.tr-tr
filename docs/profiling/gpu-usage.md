...
3. Mavi seçin **Başlat** seçtiğiniz Araçları altındaki uygulamanızı çalıştırmak için performans ve tanılama hub'ın altındaki düğmesi.  
   ...
   **Çerçeve süresi (ms)** ve **kare / saniye (FPS)** grafikler içeren iki kırmızı, yatay satırları performans hedefleyen 60 ve 30 kare / saniye temsil eder. İçinde **çerçeve süresi** grafik, uygulamanız grafiğin altına satır olduğunda performans hedef aşan ve grafik üzerine satır olduğunda eksik. İkinci grafik başına çerçeveler için tersidir - uygulamanızı grafik üzerine satır olduğunda performans hedef aşan ve grafiğin altına satır olduğunda eksik. Öncelikle, bu grafiklere uygulamanızın performansı hakkında üst düzey bir fikir edinmek ve örneğin araştırmak isteyebileceğiniz slow-downs, GPU kullanımı bir ani değişiklik ya da bir kare hızı ani düşme tanımlamak için kullanılır.  
...
Visual Studio 2017'den itibaren bu verileri ile açılabilir [GPUView](/windows-hardware/drivers/display/using-gpuview) ve [Windows Performans Çözümleyicisi](/windows-hardware/test/wpt/windows-performance-analyzer) tıklayarak **Gpuview'da Aç** veya **Aç WPA'da Aç** Tanılama oturumu alt sağ tarafta bulunan bağlantılar.
