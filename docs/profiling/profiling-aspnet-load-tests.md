---
title: ASP.NET Yükleme testlerinin profilini oluşturma | Microsoft Docs
ms.date: 11/04/2016
ms.topic: conceptual
ms.assetid: c3f5c363-be79-40b5-bfa7-db8d21378d8d
author: mikejo5000
ms.author: mikejo
manager: douge
ms.workload:
- aspnet
ms.openlocfilehash: a477d1f06e086514d164fc20fdfa582f1a28ee40
ms.sourcegitcommit: 37fb7075b0a65d2add3b137a5230767aa3266c74
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 01/02/2019
ms.locfileid: "53872992"
---
# <a name="profile-aspnet-load-tests"></a>ASP.NET yük testlerinin profilini
Toplamak profil oluşturma verileri otomatik [!INCLUDE[TCMext](../misc/includes/tcmext_md.md)] web ASP.NET Web siteleri projeleri üzerinde çalıştırdığınız testleri. Örnekleme ve katman etkileşim verileri toplayabilir. Daha fazla bilgi için aşağıdaki konulara bakın:  
  
 [(NIB) Nasıl Yapılır: Bir Web uygulamasında Web Performans Testi Düzenleyicisi'nden bir performans oturumu çalıştırma](https://msdn.microsoft.com/d14e0a27-33cc-4e4e-8792-fe03e50d51e1)
  
 [Nasıl yapılır: Visual Studio'da Test ayarlarını kullanarak yük testleri için ASP.NET Profiler'ı yapılandırma](https://msdn.microsoft.com/Library/6832fe39-04d5-4d94-8a18-3e2730bad423)  
  
## <a name="see-also"></a>Ayrıca bkz.  
 [Örnekleme veri değerlerini anlama](../profiling/understanding-sampling-data-values.md)   
 [Verileri çözümlemek için performans kurallarını kullanma](../profiling/using-performance-rules-to-analyze-data.md)   
 [Örnekleme yöntemi veri görünümleri](../profiling/profiler-sampling-method-data-views.md)   
 [Katman etkileşimleri görünümü](../profiling/tier-interactions-view.md)